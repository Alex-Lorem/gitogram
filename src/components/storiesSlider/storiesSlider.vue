<template>
  <div class="slider--wrapper">
    ...
    <splide class="slider" v-if="items.length" :options="options" :initialSlide="Number($route.params.initialSlide)">
      <splide-slide class="slider-item" v-for="item in items" :key="item.id">
          ...
          >
            <template #content>
              <div v-for="readme in readmeItems" :key="readme.id">readme.readme</div>
            </template>
...
export default {
  ...
  methods: {
    ...
    async moveSlide (index) {
      console.log(index)
    }
  },
  async created () {
    if (this.initialSlide) {
      const index = this.items.findIndex(item => item.id === this.initialSlide)
      await this.moveSlide(index)
    }
      try {
